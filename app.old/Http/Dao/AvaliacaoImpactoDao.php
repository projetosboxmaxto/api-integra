...
class AvaliacaoImpactoDao {
    
       public static function getListGridCad($filtro, $order, $order_type){
           
             $sql = "select p.* from avaliacao_impacto p where 1 = 1 ". $filtro .
       ...
       public static function salvarDadosJson( $hd_json, $ids_delete_json ){
           ...
           for ( $i = 0; $i < count($itens); $i++){
                       ...
                       $reg = new AvaliacaoImpacto();
                       
                       if ( $item_req->id != ""){
                             $reg = AvaliacaoImpacto::find($item_req->id);
                       }
					            $reg->id_impacto = $item_req->id_impacto;   
   $reg->id_cliente = ConfigDao::numeroBanco(  $item_req->id_cliente  );  
         $reg->id_materia = $item_req->id_materia;   
         $reg->tabela_materia = $item_req->tabela_materia;   
   ...
   $reg->sequencial = ConfigDao::numeroBanco(  $item_req->sequencial  );  
         $reg->id_categoria_cliente = $item_req->id_categoria_cliente;   
         $reg->cita_cliente = $item_req->cita_cliente;   
   ...
   $reg->data_materia = ConfigDao::dataBanco(  $item_req->data_materia  );  
           ...
           for ( $i = 0; $i < count($ids_delete); $i++){
               $item_req = $ids_delete[$i];
                 ...
                 if ( $item_req != ""){
                             $reg = AvaliacaoImpacto::find($item_req);
